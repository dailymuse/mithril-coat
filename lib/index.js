...
function ShadowCompiler(options) {
    options = options || {};
...
ShadowCompiler.prototype.emit = function(token) {
    this.buffer.push(token);
};

ShadowCompiler.prototype.line = function(depthDiff) {
    this.emit("\n");
...
ShadowCompiler.prototype.compiled = function() {
    return this.buffer.join("");
};

ShadowCompiler.prototype.prewalk = function(dom) {
...
ShadowCompiler.prototype.pr = function(dom, depth) {
    for(var i=0; i<dom.length; i++) {
        var el = dom[i];
        var str = "";
...
ShadowCompiler.prototype.compile = function(dom) {
    ...
    if(this.wrapFunction) {
        this.emit("function(view, controller) {");
        this.line(1);
        this.emit("return (");
    }
...
ShadowCompiler.prototype.compileDom = function(dom) {
    if(dom.length == 1) {
...
ShadowCompiler.prototype.compileElement = function(el) {
    if(el.type == "comment") {
        // Ignore comments
        return;
    }
...
ShadowCompiler.prototype.compileTag = function(el) {
...
ShadowCompiler.prototype.compileRaw = function(el) {
    this.emit("shadow.trust(");
    this.emit(el.attribs.expr);
    this.emit(")");
};

ShadowCompiler.prototype.compileHtmlTag = function(el) {
    this.emit("shadow.m(");
    ...
    for(var key in el.attribs) {
        ...
        if(key.substring(0, 5) == "expr-") {
            ...
            serializedValue = "__$shadow_expr_" + key;
        }
    ...
    for(var key in attribsSpec) {
        ...
        if(spec.expr) {
            attribsJson = attribsJson.replace(JSON.stringify("__$shadow_expr_" + key), spec.replacementValue);
        }
    }
...
ShadowCompiler.prototype.compileIf = function(el) {
    var elseIfClauses = [];
    var elseClause = null;
...
ShadowCompiler.prototype.compileMap = function(el) {
    this.emit("shadow.map(");
    this.emit(el.attribs.expr);
    this.emit(", function(");
...
ShadowCompiler.prototype.compileNbsp = function(el) {
...
ShadowCompiler.prototype.compileTemplateImport = function(el) {
    this.emit('require(');
    console.log(JSON.stringify(el.attribs.path + ".js"))
    this.emit(JSON.stringify(el.attribs.path + ".js"));
    this.emit(')(view, controller)');
};

ShadowCompiler.prototype.compileVal = function(el) {
    this.emit(el.attribs.expr);
};

ShadowCompiler.prototype.compileText = function(el) {
...
function compile(contents, options, cb) {
    ...
    var handler = new htmlparser.DomHandler(function(error, dom) {
        var compiler = new ShadowCompiler(compilerOptions);
